...
file = '/Users/gitak/Documents/capstone/교통/서울시 공영주차장 안내 정보.csv'


df = pd.read_csv(file,usecols=['주차장코드','주차장명','주소','주차장 위치 좌표 위도','주차장 위치 좌표 경도'], encoding='cp949') 

print("전처리 시작")

for i in range(len(df)):
    x_crd = float(df.loc[i, ['주차장 위치 좌표 경도']])
    y_crd = float(df.loc[i, ['주차장 위치 좌표 위도']])
...
df.to_csv("/Users/gitak/Documents/capstone/교통/parkingLot.csv",encoding='cp949')
