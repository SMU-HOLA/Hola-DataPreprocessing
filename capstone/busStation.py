...
file = '/Users/gitak/Documents/capstone/교통/busStation.csv'


df = pd.read_csv(file,usecols=['정류소번호','정류소명','버스 위치 위도','버스 위치 경도'], encoding='cp949') 

print("전처리 시작")

for i in range(len(df)):
    x_crd = float(df.loc[i, ['버스 위치 경도']])
    y_crd = float(df.loc[i, ['버스 위치 위도']])
...
df.to_csv("/Users/gitak/Documents/capstone/교통/bus_station.csv",encoding='cp949')
