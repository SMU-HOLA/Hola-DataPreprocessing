...
file = '/Users/gitak/Documents/capstone/교통/subway1.csv'


df = pd.read_csv(file,usecols=['호선','역명','지하철 코드','지하철 역 위치 경도', '지하철 역 위치 위도'], encoding='cp949') 


print("전처리 시작")

for i in range(len(df)):
    x_crd = float(df.loc[i, ['지하철 역 위치 경도']])
    y_crd = float(df.loc[i, ['지하철 역 위치 위도']])
...
df.to_csv("/Users/gitak/Documents/capstone/교통/subway_12.csv",encoding='cp949')
